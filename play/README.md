...
4. Before the Android part works, fill in `public/.well-known/assetlinks.json`. Replace `PASTE_THE_APP_SIGNING_KEY_SHA256_FROM_PLAY_CONSOLE` with the SHA-256 from Play Console → Test and release → App integrity → **App signing key certificate** (the `AB:CD:…` form).
